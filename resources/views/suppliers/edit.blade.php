...
<div class="fila">
	<div class="columna columna-1">
		<div class="title2">
			<h6>{{ __('Entidades > Proveedores > Editar') }}</h6>
		</div>
	</div>
</div>
	...
	<div id="div-ctt">
		<div class="fila">
			...
			<div class="columna columna-2">
				...
				<select name="district_id" id="district_id" required>
					...
					<option value="{{ $index }}" {{ old('district_id',$supplier->district_id) == $index ? 'selected' : '' }}>
						{{ $district }}
					</option>
					@endforeach
				</select>
		...
		<div class="fila">
			...
			<div class="columna columna-6">
				...
				<input type="tel" name="mobile" id="mobile" maxlength="11" value="{{ old('mobile',$supplier->mobile) }}" onkeypress="return checkNumber(event)" pattern="[0-9]{3} [0-9]{3} [0-9]{3}" placeholder="999 999 999" required>
			</div>
			<div class="columna columna-6">
				<p>{{ __('Teléfono fijo') }}</p>
				<input type="tel" name="phone" id="phone" maxlength="11" value="{{ old('phone',$supplier->phone) }}" onkeypress="return checkNumber(event)" pattern="[0-9]{2} [0-9]{3} [0-9]{4}" placeholder="99 999 9999">
			</div>
			<div class="columna columna-6">
				<p>{{ __('Anexo') }}</p>
				<input type="tel" name="annex" id="annex" maxlength="6" value="{{ old('annex',$supplier->annex) }}" onkeypress="return checkNumber(event)" pattern="[0-9]{4,6}" placeholder="4 a 6 dígitos">
			</div>
			<div class="columna columna-3">
