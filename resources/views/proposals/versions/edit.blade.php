...
<form method="POST" action="{{ route('proposals.updateVersion',$proposal->id) }}" role="form" id="frm-prop">
	...
	<div id="div-gen">
		...
		<div class="fila">
			...
			<div class="columna columna-5d">
				<input type="text" name="user_wholename" id="user_wholename" value="{{ old('user_wholename',($proposal->accountable->name ?? '').' '.($proposal->accountable->lastname ?? '').' ('.($proposal->accountable->code ?? '').')') }}" placeholder=" -- No hay información para mostrar -- " readonly required>
			</div>
			<div class="columna columna-5">
...
<div class="fila">
	...
	<div class="columna columna-1">
		...
		<button class="btn-effie-inv" onclick="$('#mdl-prop-apv').modal('show');"><i class="fa fa-check"></i>&nbsp;{{ __('Aprobar') }}</button>
		<button class="btn-effie-inv" onclick="$('#mdl-prop-rjt').modal('show');"><i class="fa fa-close"></i>&nbsp;{{ __('Rechazar') }}</button>
